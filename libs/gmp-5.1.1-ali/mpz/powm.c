...
void mpz_create_swt (void *sp, mpz_srcptr e)
{
	mp_create_swt ((mp_slw_t**)sp, PTR(e), SIZ(e));
}

void mpz_destroy_swt (void *sp)
{
	mp_destroy_swt ((mp_slw_t*)sp);
}

void
mpz_powm_swt (mpz_ptr r, mpz_srcptr b, void *swt, mpz_srcptr m)
{
  mp_size_t n, ncnt;
  int cnt;
  mp_ptr rp, tp;
  mp_srcptr bp, mp;
  mp_size_t rn, bn, es, itch;
  mpz_t new_b;			/* note: value lives long via 'b' */
  TMP_DECL;
  ...
  mp_size_t itch_binvert = mpn_binvert_itch (n);
  itch = n + MAX (itch_binvert, 2 * n);
  tp = TMP_ALLOC_LIMBS (itch);
  rp = tp;  tp += n;
  bp = PTR(b);

  mpn_powm_swt (rp, bp, bn, mp, n, tp, swt);
