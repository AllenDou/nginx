...
getbits (const mp_limb_t *p, mp_bitcnt_t* bi, int nbits, int* rsize)
{
  int cnt;
  mp_limb_t r;
  ...
  if (*bi <= nbits)
    ...
    {
      mp_size_t i;
      mp_bitcnt_t ebi = (*bi -= nbits);	/* bit index of low bit to extract */
      i = ebi / GMP_NUMB_BITS;		/* word index of low bit to extract */
      ebi %= GMP_NUMB_BITS;		/* bit index in low word */
      r = p[i] >> ebi;			/* extract (low) bits */
      cnt = GMP_NUMB_BITS - ebi;	/* number of bits now in r */
      if (cnt < nbits)			/* did we get enough bits? */
	r += p[i + 1] << cnt;		/* prepend bits from higher word */
      r &= (((mp_limb_t)1 << nbits) - 1);
      if (rsize) *rsize = nbits;
    }
	  ...
	  mp_srcptr mp, mp_size_t n, mp_ptr tp)
{
  ...
  expbits = getbits (ep, &ebi, windowsize, 0);
  ...
  while (ebi != 0)							\
	...
	{								\
	 ...
	 significant bit is 1.  */					\
									\
      expbits = getbits (ep, &ebi, windowsize, &this_windowsize);	\
      do								\
	{								\
	  MPN_SQR (tp, rp, n);						\
	  MPN_REDUCE (rp, tp, mp, n, mip);				\
	}								\
      while (--this_windowsize);					\
									\
...
#if 0
static void
mp_check_swt (mp_slw_t* swt)
{
	mp_slw_t* p;
	int i, j, k;
	printf ("------------\nmpn_check_swt: %p\n", swt);
	for (i = j = k = 0, p = swt; p->len; ++i, ++p) {
		printf ("%3d %u - %u\n", i, p->len, p->value);
		if (i) {
			k += p->len;
			if (p->value) {
	...
	printf ("%d bits, %d/%d pairs, %d extra multiplications\n",
		k, i, (swt->value / swt->len) << 1, j);
	for (i = 0, p = swt + 1; p->len; ++i, ++p) {
		if (p->value) {
			for (j = p->len - 1; j >= 0; --j) {
				printf ("%d", p->value & (1 << j) ? 1 : 0);
			}
		} else {
			for (j = 0; j < p->len; ++j) {
				printf ("0");
...
void
mp_create_swt (mp_slw_t **swt, mp_srcptr ep, mp_size_t en)
{
  mp_slw_t *wp;
  mp_bitcnt_t ebi;
  int len, winsize;
  ...
  MPN_SIZEINBASE_2EXP(ebi, ep, en, 1);
  winsize = win_size (ebi);
  *swt = wp = __GMP_ALLOCATE_FUNC_TYPE ((ebi / winsize) << 1, mp_slw_t);
  wp->len = winsize;
  wp->value = ebi;
  ...
  while (ebi)
    {
      ++wp;
      wp->value = getbits (ep, &ebi, winsize, &len);
      wp->len = len;

      if (ebi && !getbit (ep, ebi))
        {
          ++wp;
          wp->len = wp->value = 0;
          do
            ++wp->len;
          while (--ebi && !getbit (ep, ebi));
        }
    }
...
void
mp_destroy_swt (mp_slw_t *swt)
{
  ASSERT(swt);

  __GMP_FREE_FUNC_TYPE(swt, (swt->value / swt->len) << 1, mp_slw_t);
}

/* The same as mpn_powm except that using a pre-caculated 
   sliding windown table  */
void
mpn_powm_swt (mp_ptr rp, mp_srcptr bp, mp_size_t bn,
	mp_srcptr mp, mp_size_t n, mp_ptr tp,
	mp_slw_t *swt)
{
  mp_limb_t ip[2], *mip;
  ...
  MPN_COPY (rp, pp + n * (swt->value >> 1), n);
  ++swt;
...
#define INNERSWTLOOP							\
  for (; swt->len; ++swt)							\
	...
	{								\
      ...
      if (swt->value)							\
      	{								\
	  MPN_MUL_N (tp, rp, pp + n * (swt->value >> 1), n);		\
	  MPN_REDUCE (rp, tp, mp, n, mip);				\
	}								\
    }
