...
#define KEY_LEN_MASK 0x07
#define KEY_LEN16 0x03
#define KEY_LEN24 0x04
#define KEY_LEN32 0x05

#define IPv4    0

#define MICRO_CODE2 2
static char sha256_IV[] ={0x6a, 0x09, 0xe6, 0x67,
		    0xbb, 0x67, 0xae, 0x85,
                    0x3c, 0x6e, 0xf3, 0x72,
                    0xa5, 0x4f, 0xf5, 0x3a,
                    0x51, 0x0e, 0x52, 0x7f,
...
static char sha384_IV[] ={ 0xcb, 0xbb, 0x9d, 0x5d, 0xc1, 0x05, 0x9e, 0xd8,
...
static char sha512_IV[] ={0x6a, 0x09, 0xe6, 0x67, 0xf3, 0xbc, 0xc9, 0x08,
...
void getIV(Uint8 *ptr, AuthType auth);
...
n1_process_outbound_packet(void *device, Uint16 size, Uint16 param, Uint16 dlen,
                              ...
                              int response_order, int req_queue);
#ifdef PER_PKT_IV
int n1_get_randomIV(Uint8 *iv,int ivlen);
#endif
                         ...
                         int response_order, int req_queue)
{
   Uint32 req_id;
   int i, ret=0;
   n1_request_buffer *n1_buf = NULL;

   cavium_device *n1_device;
   n1_device = (cavium_device *)device;
   n1_buf = (n1_request_buffer*)get_buffer_from_pool (n1_device, sizeof(n1_request_buffer));
   if (n1_buf == NULL)
   {
      cavium_error ("n1_buf buffer allocation failed\n");
      return -1;
   }
...
cleanup:
   if (n1_buf != NULL)
      put_buffer_in_pool ((Uint8*)n1_buf, sizeof(n1_request_buffer));
   return ret;
}
...
cleanup:
   if (n1_buf != NULL)
      put_buffer_in_pool (n1_device, (Uint8*)n1_buf);
   return ret;
}
...
*/
   ...
   *(Uint16*)p = 0;
   ...
   p += (dir==INBOUND)?1:0;			//write at p+1 for INBOUND 
   //printk(KERN_CRIT "auth = %d\n", auth);
   if(auth >= SHA2HMAC)				//if auth is sha2,gmac
   *p |= (auth & 0x1C) >> IPSEC_ECTRL_AUTH_SHIFT;     //copy bits 2:4 of auth to 1:3 of addon ctrl byte
	 
   if(cypher >= AES128CTR)			//if cypher is ctr or gcm
   {
	   if(cypher >= AES128GCM){			//gcm
		   if(auth != GCMGMAC)
		*p |= 0x01 << IPSEC_ECTRL_GCM_SHIFT; 
	 
	   }
	   else						//ctr
        	*p |= 0x01 << IPSEC_ECTRL_CTR_SHIFT;
   }
   ...
   if(cypher != NO_CYPHER)
   {	memcpy(p, e_key, 32);   
   //set to zero the rest of 32 bytes space following (p + keylen)
   //bits 2:0 can be used to determine the key length 
	   switch(cypher & KEY_LEN_MASK)
	   {
		   case KEY_LEN16:		
		   memset(p+16 ,0, 16);		//128 bit key 
  		   break;
		   case KEY_LEN24: 
		   memset(p+24, 0, 8);		//192 bit key
		   break;
      		   case KEY_LEN32:
		   				//256 bit key
   ...
   if(next_ctx) {
      offset=next_ctx&0x7FF;
      next_ctx=next_ctx&~(0x7FF);
      next_ctx = ((struct ctx_addr *)(ptrlong)next_ctx)->phy_addr;
      *(Uint64*)p = htobe64(next_ctx+offset); 
   } else 
    *(Uint64*)p = 0; 
   ...
   if(dir == OUTBOUND)
   {
	...
	else //TRANSPORT
	{
		...
		if(udp_encap)			//in transport mode 'template' points to udp header
		{
			memcpy(p,template,8);
			p+=8;
		}
	}
	p = (Uint8*)in_buffer + 328;		//p points to 41st word from start
	len = 328;
   }
   else //INBOUND
    {   
	...
	p += 8+8+64;		//udp encap original src/dst address + expected src/dst port selectr
	len += 8+8+64;		// + expected ipv4/6 src/dst addr selector


	if(cypher>=AES128CTR)					//if aes gcm/ctr/gmac then copy nonce from last 4 bytes of e_key
	 {
		 switch(cypher & KEY_LEN_MASK)
		 {
			 case KEY_LEN16:			//128 bit key
			 memcpy(p, e_key+16, 4);
			 break;
			 case KEY_LEN24:			//192 bit key
		         memcpy(p, e_key+24, 4);        
			 break;	
			 case KEY_LEN32:			//256 bit key
			 memcpy(p, e_key+32, 4);	
			 break;
			 default:
			 break;
        ...
        p+=24;		// + 8 bytes esn mode + 8 bytes reserved
        len+=24;
   ...
   if(auth >= SHA2HMAC)		//if sha2
   {
	...
	p+=128;
	len+=128;
   }else if(dir==OUTBOUND){p+=128;len+=128;}	
...
void getIV(Uint8 *ptr, AuthType auth)
{
	...
	{
		case SHA2HMAC256:
				memcpy(ptr, sha256_IV, 32);
        	/*		*(Uint32*)(ptr+0) = htobe32(SHA256_H0);	 
				*(Uint32*)(ptr+4) = htobe32(SHA256_H1);	
				*(Uint32*)(ptr+8) = htobe32(SHA256_H2);
				*(Uint32*)(ptr+12) = htobe32(SHA256_H3);	 
				*(Uint32*)(ptr+16) = htobe32(SHA256_H4);	 
				*(Uint32*)(ptr+20) = htobe32(SHA256_H5);	 
				*(Uint32*)(ptr+24) = htobe32(SHA256_H6);	 
				*(Uint32*)(ptr+28) = htobe32(SHA256_H7);	 
				*/
				break;
		
		case SHA2HMAC384:
				memcpy(ptr, sha384_IV, 64);
				/*
...
int n1_get_randomIV(Uint8 *iv, int ivlen)
{
	...
	if(iv == NULL)
		...
		wlen = read_random_block(iv, (ivlen*8));
		//printk(KERN_CRIT "RAMDOM IV = 0x%llx\n",*((Uint64 *)iv));
		//printk(KERN_CRIT "RAMDOM IV = 0x%llx\n",*(((Uint64 *)iv+1)));
	}	
	return (wlen/8);
}
