...
CspDmaMode global_dma_mode=CAVIUM_DIRECT;
...
{
   n1_request_buffer *buffer = NULL;
   Uint32 req_id =0;
   int i =0, ret=0;
   cavium_device *n1_device;
   n1_device = (cavium_device *)device;
   buffer = (n1_request_buffer*)get_buffer_from_pool (n1_device, sizeof(n1_request_buffer));
   if (buffer == NULL)
   {
      cavium_error ("buffer allocation failed\n");
      return -1;
   }
   ...
   if (buffer != NULL)
      put_buffer_in_pool (n1_device, (Uint8*)buffer);
    return ret;
}
