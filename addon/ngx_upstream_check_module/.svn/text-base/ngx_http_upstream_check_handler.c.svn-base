...
static ngx_int_t ngx_http_upstream_check_client_find_host(ngx_http_check_peer_shm_t  *peer_shm);
static ngx_int_t ngx_http_upstream_check_client_init_shm_zone(ngx_shm_zone_t *shm_zone, void *data);  
static ngx_int_t ngx_http_upstream_check_lb_init_shm_zone(ngx_shm_zone_t *shm_zone, void *data);      
static ngx_int_t ngx_http_upstream_check_client_write_file(ngx_http_request_t *r);
...
static ngx_http_check_status_t  ngx_check_http_expect_not_alive_status[] = {
    { NGX_CHECK_HTTP_NOT_ALIVE_400, 400},
    { NGX_CHECK_HTTP_NOT_ALIVE_403, 403},
    { NGX_CHECK_HTTP_NOT_ALIVE_404, 404},
    { NGX_CHECK_HTTP_NOT_ALIVE_408, 408},
    { NGX_CHECK_HTTP_NOT_ALIVE_499, 499},   
    { NGX_CHECK_HTTP_NOT_ALIVE_500, 500},
    { NGX_CHECK_HTTP_NOT_ALIVE_502, 502},
    { NGX_CHECK_HTTP_NOT_ALIVE_503, 503},
    { NGX_CHECK_HTTP_NOT_ALIVE_504, 504},
    { NGX_CHECK_HTTP_NOT_ALIVE_ERR, 0}
};
...
static ngx_list_t  *check_peers_list = NULL;
...
ngx_http_check_add_timers(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < peers->peers.nelts; i++) {
        ...
        if (check_peers_list != NULL){    //add by shengyan 2012/12/10
            continue;	
        }
...
ngx_http_check_http_parse(ngx_http_check_peer_t *peer)
{
    ...
    if ((ctx->recv.last - ctx->recv.pos) > 0) {
        ...
        if (ucscf->status_alive_flag == 1){
        	 ...
        	 code_n = NGX_CHECK_HTTP_NOT_ALIVE_ERR;      	 
        	 for (i=0; ngx_check_http_expect_not_alive_status[i].status != 0; i++){
        	    if (code == ngx_check_http_expect_not_alive_status[i].status){
        	    	code_n = ngx_check_http_expect_not_alive_status[i].index;
        	      break;	
        	    }	
        	 }        	        	
        }                
        ...
        if (ucscf->status_alive_flag == 1){
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       "http_parse: code_n: %i, status_alive: %ui",
                       code_n, ucscf->code.status_alive);
        }else{
        	  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       "http_parse: code_n: %i, status_not_alive: %ui",
                       code_n, ucscf->code.status_not_alive);
        }               
                
        if (ucscf->status_alive_flag == 1){  //add by shengyan 2012/11/14
            if (code_n & ucscf->code.status_alive) {
                return NGX_OK;
            } else {
                return NGX_ERROR;
            }
        }else{                              //add by shengyan 2012/11/14
            if (code_n & ucscf->code.status_not_alive) {
                return NGX_ERROR;
            } else {
                return NGX_OK;
            }	        	
...
{	  
    ...
    if (peers == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check write file] can not find the check servers, "
                      "have you added the check servers? ");

        return NGX_ERROR;
    }
        
    ucmcf = ngx_http_get_module_main_conf(r, ngx_http_upstream_check_module);
    file = ucmcf->global_check_client_file.data;   
        
    fp = fopen(file, "w+");
    if (fp == NULL){
    	  ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check write file] can not fopen the file, permission denied");
        return NGX_ERROR;
    }
    ...
    for (i = 0; i < peers->peers.nelts; i++) {
    	  if(peer_shm[i].down){            
    	  	  fwrite(peer[i].upstream_name->data, sizeof(u_char), peer[i].upstream_name->len, fp);
    	  	  fwrite(" ", sizeof(u_char), 1, fp);
    	  	  fwrite(peer[i].peer_addr->name.data, sizeof(u_char), peer[i].peer_addr->name.len, fp);   	  
    	  	  fwrite("\n", sizeof(u_char), 1, fp);	
    	  }    	  
    }
    fflush(fp);
...
ngx_http_upstream_check_client_find_host(ngx_http_check_peer_shm_t  *peer_shm)
{
	  ...
	  ngx_http_check_addr_t  *addr;

	  list = check_peers_list;
	  if (list == NULL){
		   return NGX_ERROR;		
	  }
	  
	  part = &(list->part);
	  addr = (ngx_http_check_addr_t *)part->elts;
	  ...
	  for (i=0; ; i++){
	      ...
	      if (addr[i].name.len != peer_shm->name.len) {
            continue;
        }
        
        if (addr[i].upstream_name.len != peer_shm->upstream_name.len) {
            continue;
        }        
        
        if (ngx_strncmp(addr[i].upstream_name.data, peer_shm->upstream_name.data, peer_shm->upstream_name.len) != 0){
            continue;	
        }
        
        if (ngx_strncmp(addr[i].name.data, peer_shm->name.data, peer_shm->name.len) == 0){
            return NGX_OK;	
        }
...
ngx_http_upstream_check_client_init_shm_zone(ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    for (i = 0; i < number; i++) {
        ...
        peer_shm->name.len = peer[i].peer_addr->name.len;
        peer_shm->name.data = ngx_slab_alloc(shpool, peer_shm->name.len);
        if (peer_shm->name.data == NULL){
            goto failure;   	
        }
        ngx_memcpy(peer_shm->name.data, peer[i].peer_addr->name.data,
                   peer_shm->name.len);
        
        //add by shengyan 2013/02/19
        peer_shm->upstream_name.len = peer[i].upstream_name->len;
        peer_shm->upstream_name.data = ngx_slab_alloc(shpool, peer_shm->upstream_name.len);
        if (peer_shm->upstream_name.data == NULL){
            goto failure;   	
        }
        ngx_memcpy(peer_shm->upstream_name.data, peer[i].upstream_name->data,
                   peer_shm->upstream_name.len);
        //add by shengyan 2013/02/19
                   
        ucscf = peer[i].conf;
        if (ngx_http_upstream_check_client_find_host(peer_shm) == NGX_OK){
            ngx_http_check_set_shm_peer(peer_shm, NULL, 1);   //1 down, 0 up	
            ngx_log_error(NGX_LOG_ALERT, shm_zone->shm.log, 0,
                  "[check_client_init_shm_zone] set status down %V in upstream %V", &peer_shm->name, peer[i].upstream_name);
        }else{
            ngx_http_check_set_shm_peer(peer_shm, NULL, 0);   //1 down, 0 up	
        }
...
ngx_http_upstream_check_init_shm(ngx_conf_t *cf, void *conf)
{
    ...
    if (ucmcf->peers->peers.nelts > 0) {
        ...
        if (ucmcf->global_check_client_mode == 1){
        	  check_peers_list = ucmcf->global_check_client_list;
        	  shm_zone->init = ngx_http_upstream_check_client_init_shm_zone;
        }else{
        	  check_peers_list = NULL;
        	  shm_zone->init = ngx_http_upstream_check_lb_init_shm_zone;
        }               
    }else {
...
ngx_http_upstream_check_status_handler(ngx_http_request_t *r)
{
    ...
    if (peers == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check status] can not find the check servers, "
                      "have you added the check servers? ");
    ...
    b->last = ngx_snprintf(b->last, b->end - b->last,
        "{\"title\":\"nginx http upstream check status\",\n"
            "\"check_upstream_server_number\":\"%ui\",\n"
            "\"shm_name\":\"%ui\",\n"
            "\"statuslist\":[\n",    
	peers->peers.nelts, ngx_http_check_shm_generation);

    for (i = 0; i < peers->peers.nelts; i++) {
    	  if (check_peers_list != NULL){
    	  	  b->last = ngx_snprintf(b->last, b->end - b->last,
                            "{\n"
                            "\"index\":\"%ui\","
                            "\"upstream\":\"%V\","
...
ngx_http_upstream_check_enable_handler(ngx_http_request_t *r)
{
    ...
    ngx_str_t                      host;
    ngx_url_t                      u; 
    ngx_http_variable_value_t      *vv_host, *vv_upstream;
    ngx_http_upstream_check_loc_conf_t       *clcf;
    ...
    if (peers == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check enable] can not find the check servers, "
                      "have you added the check servers? ");
    ...
    if (check_peers_list == NULL){
    	  ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check enable] mode does not support the handler");
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }    
    
    upstream_flag = 1;
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_upstream_check_module);
    //inverse upstream
    if (clcf->upstream_index != -1){
        vv_upstream = ngx_http_get_indexed_variable(r, clcf->upstream_index);
        if (vv_upstream == NULL || vv_upstream->not_found || vv_upstream->len == 0) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check enable] can't find upstream variable, enable all hosts meeted the conditions!");
            upstream_flag = 0;
        }
    }
        
    //inverse host
    vv_host = ngx_http_get_indexed_variable(r, clcf->host_index);
    if (vv_host == NULL || vv_host->not_found) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check enable] can't find host variable");
        return NGX_HTTP_BAD_REQUEST;
    }

    if (vv_host->len == 0) {
    	  ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check enable] handler need host variable");
        return NGX_HTTP_BAD_REQUEST;
    }
    
    ngx_memzero(&u, sizeof(ngx_url_t)); 
    u.url.len = vv_host->len;
    u.url.data = vv_host->data;
    u.default_port = 80;
    if (ngx_parse_url(r->pool, &u) != NGX_OK) {
        if (u.err) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check enable] can't parse the handler args");               
        }
        return NGX_HTTP_BAD_REQUEST;
    }
    host.data = u.addrs->name.data;
    host.len = u.addrs->name.len;
    ...
    b->last = ngx_snprintf(b->last, b->end - b->last,
        "{\"title\":\"nginx http upstream check enable\",\n"
            "\"check_upstream_server_number\":\"%ui\",\n"
            "\"shm_name\":\"%ui\",\n"
            "\"statuslist\":[\n",    
	  peers->peers.nelts, ngx_http_check_shm_generation);
    ...
    for (i = 0; i < peers->peers.nelts; i++) {
    	  if (host.len != peer[i].peer_addr->name.len){
    	      continue;	    	  	
    	  }
    	  
    	  if (clcf->upstream_index != -1 && upstream_flag != 0 && vv_upstream->len != 0){   //add by shengyan 2013/02/19
    	  	  if (ngx_strncmp(vv_upstream->data, peer[i].upstream_name->data, peer[i].upstream_name->len) != 0){
    	  	  	 continue;
    	  	  }
    	  }	
    	  	  
    	  if (ngx_strncmp(host.data, peer[i].peer_addr->name.data, host.len) == 0){
    	  	      ngx_http_check_client_status_update(&peer[i], 1); //1=UP, 0=down
    	  	      b->last = ngx_snprintf(b->last, b->end - b->last,
                            "{\n"
                            "\"index\":\"%ui\","
                            "\"upstream\":\"%V\","
                ...
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check enable] set status up %V in upstream %V", &peer[i].peer_addr->name, peer[i].upstream_name);
    	 }    	  
   }
    b->last = ngx_snprintf(b->last, b->end - b->last,
             "]\n}\n");

    ngx_http_upstream_check_client_write_file(r);
...
ngx_http_upstream_check_disable_handler(ngx_http_request_t *r)
{
    ...
    if (peers == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check disable] can not find the check servers, "
                      "have you added the check servers? ");
    ...
    if (check_peers_list == NULL){
    	  ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check disable] mode does not support the handler");
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }    
    ...
    if (clcf->upstream_index != -1){
        ...
        if (vv_upstream == NULL || vv_upstream->not_found || vv_upstream->len == 0) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check disable] can't find upstream variable, disable all hosts meeted the conditions!");
            upstream_flag = 0;
        }	
    }
    ...
    if (vv_host == NULL || vv_host->not_found) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check disable] can't find host variable");
        return NGX_HTTP_BAD_REQUEST;
    }

    if (vv_host->len == 0) {
    	  ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check disable] handler need host variable");
        return NGX_HTTP_BAD_REQUEST;
    }
    ...
    if (ngx_parse_url(r->pool, &u) != NGX_OK) {
        if (u.err) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check disable] can't parse the handler args");               
        }
        return NGX_HTTP_BAD_REQUEST;
    }
    host.data = u.addrs->name.data;
    ...
    b->last = ngx_snprintf(b->last, b->end - b->last,
        "{\"title\":\"nginx http upstream check disable\",\n"
            "\"check_upstream_server_number\":\"%ui\",\n"
            "\"shm_name\":\"%ui\",\n"
            "\"statuslist\":[\n",    
	  peers->peers.nelts, ngx_http_check_shm_generation);
    ...
    for (i = 0; i < peers->peers.nelts; i++) {
    	  ...
    	  if (ngx_strncmp(host.data, peer[i].peer_addr->name.data, host.len) == 0){
    	  	  ngx_http_check_client_status_update(&peer[i], 0);   //1=UP, 0=down
    	  	  b->last = ngx_snprintf(b->last, b->end - b->last,
                            "{\n"
                            "\"index\":\"%ui\","
                            "\"upstream\":\"%V\","
            ...
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "[http upstream check disable] set status down %V in upstream", &peer[i].peer_addr->name, peer[i].upstream_name);
    	  }
